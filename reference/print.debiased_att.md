...
high-dimensional (`p >= NT`) regime it also surfaces the experimental
caveat and the nodewise-direction diagnostics (`lambda_c` /
`lambda_node`, KKT feasibility, convergence) the documentation tells
users to inspect; the fixed-`p` print shows only the estimate block.
