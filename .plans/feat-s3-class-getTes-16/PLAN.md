...
- **2026-05-10 (implementation + post-execution review)** — Implementation committed as `3688aff` (12 files, +310/-38 lines net). Per-PR CRAN gate verified clean: `--as-cran` 0/0/0, 1065/1065 tests pass, `spell_check` empty (after 36-term WORDLIST bootstrap), `url_check` clean modulo 2 pre-existing redirects in vignettes. Post-execution review subagent verdict: LGTM, no blockers, no action items. Workflow doc cleanup applied during this round: `.workflow/PLANS.md:171` "preserve tabs by hand" warning corrected — `air format .` is empirically a no-op for tab-vs-space conversion in this repo.

- **2026-05-10 (air 0.5.0 → 0.9.0 upgrade + bundled reformat)** — Greg directed an air upgrade. After the upgrade (`~/.local/bin/air` 0.5.0 → 0.9.0; backup at `air.0.5.0.bak`), `air format .` added braces around single-statement `if` bodies in 7 pre-existing R files (`convert_dfs.R`, `core_funcs.R`, `etwfe_class.R`, `etwfe_core.R`, `fetwfe_class.R`, `gen_funcs.R`, `utility.R`) — a brace-style enforcement that 0.5.0 didn't apply. ~62 lines of pure cosmetic diff (no behavior changes). Greg directed bundling the reformat into this PR rather than splitting. Per-PR CRAN gate re-verified clean after the reformat: `--as-cran` 0/0/0, tests 1065/1065, spell_check empty, url_check unchanged. Reformat committed as a separate chore commit on top of `3688aff` and `161f53d`. Future PRs use 0.9.0 conventions (braces always).

- **2026-05-10 (round-2 post-execution review + repo-local air.toml)** — Round-2 post-execution review (focused on the chore commit `a64fb35`) verdict: LGTM, no findings. Subagent surfaced the long-running mystery: air walks UP the directory tree looking for `.air.toml`, and Greg has a user-global `~/.air.toml` setting `indent-style = "tab"` that's been driving the repo's formatting all along. The earlier `find -name "air.toml"` search missed it because of the leading dot. Greg directed adding a repo-local `air.toml` at the root (4 lines, mirrors `~/.air.toml` settings) so the convention is independent of the user-global config and travels with the repo. WORKFLOW_LESSONS.md §13 rewritten to drop the "mystery" framing and document the `.air.toml` upward-search mechanism. PLANS.md and R_WORKFLOW.md updated to reference the repo-local config. The `^air\.toml$` line in `.Rbuildignore` (added in this PR's Step 4b) keeps `air.toml` out of the CRAN-built source tarball.
