...
# ------------------------------------------------------------------------------
# 4. SVD-bound warning: a D_N outside the event-study singular-value lemma's
#    bounds warns but still returns a finite fit.
# ------------------------------------------------------------------------------

test_that("an out-of-bounds fusion_matrix warns but still fits", {
	skip_on_cran()
	sim <- .fm236_sim()
	fit <- .fm236_fit(sim)
	nt <- length(fit$treat_inds)

	# sigma_max = 10 > sqrt(6): a scaled identity blows the upper bound.
	big <- diag(nt) * 10

	# NB: not via .fm236_fit() -- that wraps in suppressWarnings(), which would
	# swallow the very warning under test. Call fetwfeWithSimulatedData()
	# directly (other, unrelated warnings are tolerated by expect_warning).
	expect_warning(
	...
	fit_w <- .fm236_fit(sim, fusion_matrix = big)
