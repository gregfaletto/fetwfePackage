...
test_that("simulateDataCore (without interactions) returns expected output", {
  ...
  res <- simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta_int, seed = 123, gen_ints = FALSE)
...
test_that("simulateDataCore is reproducible with same seed", {
  ...
  res1 <- simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta_int, seed = 456, gen_ints = FALSE)
  res2 <- simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta_int, seed = 456, gen_ints = FALSE)
...
test_that("simulateDataCore errors when beta has wrong length (with interactions)", {
  ...
  expect_error(
    simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta_wrong, seed = 123, gen_ints = TRUE),
    "length\\(beta\\) must be"
  )
})
...
test_that("simulateDataCore errors when beta has wrong length (without interactions)", {
  ...
  expect_error(
    simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta_wrong, seed = 123, gen_ints = FALSE),
    "length\\(beta\\) must be"
  )
})
...
test_that("simulateDataCore returns non-missing X and y", {
  ...
  res <- simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta_int, seed = 789, gen_ints = FALSE)
...
test_that("Covariates are uniformly bounded when distribution = 'uniform'", {
  ...
  sim_data <- simulateDataCore(N, T_val, R_val, d_val, sig_eps_sq, sig_eps_c_sq, beta,
...
test_that("simulateDataCore errors when R >= T", {
  ...
  expect_error(
    simulateDataCore(N, T_val, R_val, d_val, 1, 1, beta, seed = 123, 
...
test_that("Input from genCoefsCore() works", {
  ...
  res <- genCoefsCore(R=R_val, T=T_val, d=d_val, density=density, eff_size=eff_size)

  p_int <- compute_p_int(T=T_val, R=R_val, d=d_val)

  expect_equal(p_int, length(res$beta))
  
  sim_data <- simulateDataCore(N=N, T=T_val, R=R_val, d=d_val, sig_eps_sq=sig_eps_sq,
