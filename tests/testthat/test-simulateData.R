...
test_that("simulateData (with interactions) returns expected output", {
  ...
  res <- obj |> simulateData(N=N, sig_eps_sq=sig_eps_sq, sig_eps_c_sq=sig_eps_c_sq)
  
  # Check list names (X now instead of X_int)
  expected_names <- c("pdata", "X", "y", "covs", "time_var", "unit_var", "treatment",
    "response", "coefs",  "first_inds", "N_UNTREATED", "assignments", "indep_counts",
    "p", "N", "T", "R", "d", "sig_eps_sq", "sig_eps_c_sq")
...
test_that("simulateData is reproducible with same seed", {
  ...
  coefs_obj <- genCoefs(R = R_val, T = T_val, d = d_val, density = 0.1, eff_size = 2, seed = 456)
  
  res1 <- coefs_obj |> simulateData(N=N, sig_eps_sq=sig_eps_sq, sig_eps_c_sq=sig_eps_c_sq)
  res2 <- coefs_obj |> simulateData(N=N, sig_eps_sq=sig_eps_sq, sig_eps_c_sq=sig_eps_c_sq)
...
test_that("simulateData errors when beta has wrong length", {
  ...
  bad_coefs <- list(beta = beta_wrong, R = R_val, T = T_val, d = d_val, seed = 123)
  class(bad_coefs) <- "FETWFE_coefs"
  
  expect_error(
    simulateData(coefs_obj=bad_coefs, N=N, sig_eps_sq=sig_eps_sq, sig_eps_c_sq=sig_eps_c_sq),
    "length\\(beta\\)"
  )
})
...
test_that("Output from simulateData can be piped into fetwfeWithSimulatedData", {
  ...
  sim_data <- coefs_obj |> simulateData(N = 120, sig_eps_sq = 5, sig_eps_c_sq = 5)
...
# ------------------------------------------------------------------------------
test_that("Output from simulateData can be piped into fetwfeWithSimulatedData", {
  coefs_obj <- genCoefs(R = 5, T = 30, d = 0, density = 0.1, eff_size = 2, seed = 123)
  sim_data <- coefs_obj |> simulateData(N = 120, sig_eps_sq = 5, sig_eps_c_sq = 5)
  result <- sim_data |> fetwfeWithSimulatedData()
  expect_type(result, "list")
  expect_true("att_hat" %in% names(result))
...
test_that("Covariates are uniformly bounded when distribution = 'uniform'", {
  ...
  coefs_obj <- genCoefs(R = R_val, T = T_val, d = d_val, density = 0.1, eff_size = 2, seed = 123)
  
  sim_data <- coefs_obj |> simulateData(N=N, sig_eps_sq=sig_eps_sq, sig_eps_c_sq=sig_eps_c_sq,
