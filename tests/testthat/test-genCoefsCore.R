...
test_that("genCoefsCore is reproducible with the same seed", {
  res1 <- genCoefsCore(R = 5, T = 30, density = 0.1, eff_size = 2, d = 12, seed=123)
  
  res2 <- genCoefsCore(R = 5, T = 30, density = 0.1, eff_size = 2, d = 12, seed=123)
...
test_that("beta from genCoefsCore is a valid input for simulateDataCore", {
  ...
  sim_data <- simulateDataCore(N = N, T = T, R = R, d = d,
...
test_that("simulateDataCore and fetwfe work when d = 0", {
  ...
  coefs <- genCoefsCore(R = R_val, T = T_val, density = 0.1, eff_size = 2, d = d_val)
