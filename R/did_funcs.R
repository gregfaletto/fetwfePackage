...
# getIntsMat <- function(X, covs, time_var_names, cohort_vars,
#     cohort_treat_names){

#     # Create formula: first base terms
#     formula1 <- paste("~", colnames(X)[1])
#     formula2 <- paste(colnames(X)[2:ncol(X)], collapse=" + ")

#     #### Add interactions

#     # Covariates interacted with year dummies
#     time_int_terms <- do.call(paste_func, expand.grid(covs, time_var_names))  
#     p <- length(covs)
#     T <- length(time_var_names)
#     stopifnot(length(time_int_terms) == T*p)

#     time_int_formula <- paste(time_int_terms, collapse=" + ")

#     # Covariates interacted with cohort dummies
#     cohort_int_terms <- do.call(paste_func, expand.grid(covs, cohort_vars))

#     n_cohorts <- length(cohort_vars)
#     stopifnot(length(cohort_int_terms) == n_cohorts*p)

#     cohort_int_formula <- paste(cohort_int_terms, collapse=" + ")

#     # Demeaned covariates interacted with treatment dummies
#     demeaned_covs <- scale(X[, covs], center=TRUE, scale=FALSE)
#     demeaned_cov_names <- paste(colnames(demeaned_covs), "centered", sep="_")

#     stopifnot(length(demeaned_cov_names) == p)
#     stopifnot(ncol(demeaned_covs) == p)

#     colnames(demeaned_covs) <- demeaned_cov_names
#     X <- data.frame(X, demeaned_covs)

#     treatment_int_terms <- do.call(paste_func, expand.grid(demeaned_cov_names,
#         unlist(cohort_treat_names)))

#     num_treats <- length(unlist(cohort_treat_names))
#     stopifnot(length(treatment_int_terms) == p*num_treats)

#     treatment_int_formula <- paste(treatment_int_terms, collapse=" + ")

#     formula1_obj <- paste(formula1, formula2, time_int_formula,
#         cohort_int_formula, treatment_int_formula, sep=" + ")

#     formula_ints <- formula(formula1_obj)

#     print("Forming model.matrix with all interactions...")
#     t0 <- Sys.time()

#     X_ints <- model.matrix(formula_ints, X)
#     X_ints <- X_ints[, colnames(X_ints) != "(Intercept)"]
...
# sparseVec <- function(length){
#     vec <- rep(0, length)

#     nonzero_entries <- which(as.logical(rbinom(n=length, size=1,
#         prob=SPARSITY)))

#     for(j in nonzero_entries){
#         vec[j] <- rfunc(1)*EFF_SIZE
#     }

#     return(vec)
# }

# sparseFusedVec <- function(length, start_value=NA){

#     if(!is.na(start_value)){
#         vec <- rep(start_value, length)
#     } else{
#         # Decide whether baseline should be 0 or EFF_SIZE
#         vec <- rep(0, length)

#         if(runif(1) < SPARSITY){
#             vec <- rep(rfunc(1)*EFF_SIZE, length)
#         }
#     }

#     num_diffs <- rbinom(n=1, size=length - 1, prob=SPARSITY)

#     if(num_diffs > 0){
#         for(i in 2:(1 + num_diffs)){
#             vec[i:length] <- vec[i:length] + rfunc(1)*EFF_SIZE
#         }
#     }
#     stopifnot(length(vec) == length)
#     return(vec)
# }

# sparseFusedTreatVec <- function(num_treats, first_treats){
#     vec <- rep(as.numeric(NA), num_treats)

#     first_treat_effects <- sparseFusedVec(R)

#     vec[first_treats] <- first_treat_effects

#     for(r in 1:R){
#         # Which labels in vec will we modify?
#         inds_r <- which(TREAT_LABELS == r)

#         stopifnot(min(inds_r) == first_treats[r])
#         stopifnot(length(inds_r) == T - r) 

#         vec[inds_r] <- sparseFusedVec(T - r, vec[min(inds_r)])
#     }
...
#     theta_inds <- which(as.logical(rbinom(n=p, size=1, prob=SPARSITY)))
#     num_coefs <- length(theta_inds)
#     signs <- rfunc(num_coefs)
...
#     beta[1:R] <- genBackwardsInvFusionTransformMatDataApp(R) %*% theta[1:R]

#     stopifnot(all(is.na(beta[(R + 1):(R + T - 1)])))
#     beta[(R + 1):(R + T - 1)] <- genBackwardsInvFusionTransformMatDataApp(T - 1) %*%
#         theta[(R + 1):(R + T - 1)]
...
#         beta[inds_j] <- genBackwardsInvFusionTransformMatDataApp(R) %*% theta[inds_j]
#     }
...
#         beta[inds_j] <- genBackwardsInvFusionTransformMatDataApp(T - 1) %*% theta[inds_j]
#     }
...
#         # For each cohort, we have N_PER_COHORT*T rows of observations which
#         # need to be assigned a cohort fixed effect. Identify those rows:
#         first_ind_r <- N_UNTREATED*T + (r - 1)*N_PER_COHORT*T + 1
#         last_ind_r <- first_ind_r + N_PER_COHORT*T - 1

#         stopifnot(last_ind_r == N_UNTREATED*T + r*N_PER_COHORT*T)
#         stopifnot(length(first_ind_r:last_ind_r) == N_PER_COHORT*T)
#         stopifnot(all(cohort_fe[first_ind_r:last_ind_r, ] == 0))

#         # Now add cohort fixed effects in the rth column for these rows
#         cohort_fe[first_ind_r:last_ind_r, r] <- rep(1, N_PER_COHORT*T)
#     }

#     # stopifnot(min(abs(cov(cohort_fe))) > 0)

#     # Time fixed effects: only do 1 through T - 1.
#     time_fe <- matrix(0, N*T, T - 1)
#     # No fixed effect for first time
#     for(t in 1:(T - 1)){
#         stopifnot(all(time_fe[, t] == 0))
#         # For each time (except the last), we have N rows of observations which
#         # need to be assigned a time fixed effect. Identify those rows:
#         # The tth row of every block should have a 1 in the tth column
#         rows_t <- (0:(N - 1))*T + t
#         stopifnot(length(rows_t) == N)
#         time_fe[rows_t, t] <- rep(1, N)
#     }

#     return(list(cohort_fe=cohort_fe, time_fe=time_fe))
# }
...
# genXY <- function(num_treats){

#     # Generate cohort fixed effects and time fixed effects according to
#     # parameters, and generate covariates randomly

#     res <- generateBaseEffects()

#     cohort_fe <- res$cohort_fe
#     time_fe <- res$time_fe
#     X_long <- res$X_long
...
#     X_int <- cbind(X_int, X_long_cohort, X_long_time)

#     rm(res)

#     # Generate treatment effects
#     res <- genTreatVarsSim(num_treats)

#     treat_mat_long <- res$treat_mat_long
...
#         N=N, T=T, R=R, d=d, N_UNTREATED=N_UNTREATED)
...
#     # Generate model coefficients (p-vector)
#     coefs <- genCoefs(num_treats)

#     # Generate unit random effects
#     unit_res <- rnorm(N, mean=0, sd=sqrt(SIG_EPS_C_SQ))

#     # Generate response (no intercept)
#     y <- X_int %*% coefs + rep(unit_res, each=T) + rnorm(N*T, mean=0,
#         sd=sqrt(SIG_EPS_SQ))

#     # Center y
#     y <- y - mean(y)

#     return(list(X_int=X_int, y=y, coefs=coefs, first_inds=first_inds))
# }


# genTransformedMatFusion <- function(X_sub){
#     # For a set of k features that have a fusion penalty (first feature
#     # penalized directly, remaining features penalize difference from previous
#     # feature), returns the transformed matrix needed for fusion penalized
#     # regression
#     stopifnot(nrow(X_sub) == N*T)

#     k <- ncol(X_sub)

#     X_sub_mod <- matrix(as.numeric(NA), N*T, k)
#     for(j in 1:k){
#         if(j < k){
#             X_sub_mod[, j] <- rowSums(X_sub[, j:k])
#         } else{
#             X_sub_mod[, j] <- X_sub[, k]
#         }
#     }
#     stopifnot(all(!is.na(X_sub_mod)))

#     return(X_sub_mod)
# }


# genTransformedMatTwoWayFusion <- function(X_sub, first_inds){
#     # For a set of k treatment effects from R cohorts that have a fusion penalty
#     # (first feature penalized directly, remaining features penalize difference
#     # from previous feature) within each cohort and also a fusion penalty of the
#     # first treatment effect in each cohort towards the first treatment effect
#     # from the previous cohort, returns the transformed matrix needed for fusion
#     # penalized regression. The argument first_inds should consist of the
#     # indices of the columns in X_sub corresponding to the first treatment
#     # effect in each cohort. Within each cohort, the treatment effects should
#     # be arranged to go forward in time. The cohorts should also be arranged
...
#             X_sub_mod[, j] <- X_sub[, k]
#         }
#         else if(j %in% first_inds){
#             X_sub_mod[, j] <- rowSums(X_sub[, j:k])
#         }
#         else{
#             stopifnot(j < k)
#             next_inds <- first_inds[first_inds > j]
#             next_ind <- min(next_inds)

#             stopifnot(next_ind > j)
#             stopifnot(next_ind %in% first_inds)

#             X_sub_mod[, j] <- rowSums(X_sub[, j:next_ind])
#         }
#     }
#     stopifnot(all(!is.na(X_sub_mod)))
...
#     stopifnot(all(first_inds %in% 1:k))
#     stopifnot(length(first_inds) == length(unique(first_inds)))
#     stopifnot(identical(first_inds, sort(first_inds)))
#     stopifnot(length(first_inds) == R)

#     # First construct a typical fusion differences matrix
...
#     # feature the first treatment effect of each group fuses toward--fusing
#     # towards the first treatment effect in the previous cohort rather than
#     # the last treatment effect in the previous cohort.
...
# untransformVecFusion <- function(coef_vec, len){
#     stopifnot(length(coef_vec) == len)
#     ret_vec <- rep(as.numeric(NA), len)
#     for(j in 1:len){
#         ret_vec[j] <- sum(coef_vec[1:j])
#     }
#     stopifnot(all(!is.na(ret_vec)))
#     return(ret_vec)
# }

# untransformTwoWayFusionCoefs <- function(coef_vec, first_inds, len){
#     stopifnot(length(coef_vec) == len)
#     stopifnot(all(first_inds %in% 1:len))
#     stopifnot(length(first_inds) == R)

#     ret_vec <- rep(as.numeric(NA), len)
#     for(j in 1:num_treats){
#         ret_vec[first_inds[j]] <- sum(coef_vec[first_inds[1:j]])
#     }
#     stopifnot(all(!is.na(ret_vec[first_inds])))
#     stopifnot(all(is.na(ret_vec[setdiff(1:len, first_inds)])))
...
#         for(k in (first_ind_j + 1):last_ind_j){
...
# genFusionTransformMat <- function(n_vars){
#     # Generates D matrix in relation theta = D beta, where D beta is what
#     # we want to penalize (for a single set of coefficients where we want to
#     # penalize first coefficient directly and penalize remaining coefficints
...
estOmegaSqrtInv <- function(y, X_ints, N, T, p){
    ...
    # Get residuals
    y_hat_fe <- glmnet::predict.cv.glmnet(lin_mod_fe, s="lambda.min", newx=X_ints_fe)

    # Get coefficients
    beta_hat_fe <- glmnet::coef.cv.glmnet(lin_mod_fe, s="lambda.min")[2:(p + 1)]
