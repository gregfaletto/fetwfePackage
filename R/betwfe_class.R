...
print.betwfe <- function(
	...
	# show_internal block reads top-level fields (matches etwfe's flat output
	# shape, NOT fetwfe's $internal-nested shape -- see Decision Log in
	# .plans/feat-etwfe-betwfe-vignette-17/PLAN.md for rationale).
	if (show_internal) {
		cat("\nInternal Details:\n")
		cat(
			"  X dims     :",
...
summary.betwfe <- function(object, full_catt = FALSE, ...) {
	list(
		...
		structure(class = "summary.betwfe")
}
