...
#' twfeCovs OLS estimator core (thin wrapper over `.ols_estimator_core()`)
#'
#' Forwards to [.ols_estimator_core()] with the collapsed one-effect-per-cohort
#' twfeCovs basis (`is_twfe_covs = TRUE`); see that function for the full
#' contract and return value. Unified with `etwfe_core()` in #327. Retained as a
#' named function because `twfeCovs()` passes it as `core_fn` to
#' `.assemble_ols_estimator()`.
#' @keywords internal
#' @noRd
twfeCovs_core <- function(...) {
	.ols_estimator_core(..., is_twfe_covs = TRUE)
}
