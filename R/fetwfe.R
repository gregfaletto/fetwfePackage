...
#'   stops). A `D_N` whose singular values satisfy `sigma_max <= sqrt(6)` and
#'   `sigma_min >= 1 / (T * sqrt(2 * T))` inherits the paper's inferential
#'   guarantees (Faletto 2025, event-study singular-value lemma); outside those
#'   bounds the fit stays a valid point estimator but `fetwfe()` emits a
#'   `warning()`. Default is `NULL` (use the built-in `fusion_structure`).
...
#' \item{R}{Deprecated alias for \code{G}, retained for backward compatibility; populated with the same value. Use \code{G}. Will be removed in a future release.}
