...
The paper's inferential guarantees (valid standard errors and confidence
intervals) hold for any `D_N` whose singular values satisfy
`sigma_max <= sqrt(6)` and `sigma_min >= 1 / (T * sqrt(2 * T))` (Faletto 2025,
event-study singular-value lemma); both built-ins satisfy these bounds. If you
supply a `D_N` **outside** those bounds, `fetwfe()` still returns a valid point
estimator but emits a `warning()` that the inferential guarantees may not hold:

```{r, warning = TRUE}
# A poorly scaled D_N (sigma_max far above sqrt(6)) triggers the bounds warning.
out_of_bounds <- diag(num_treats) * 10
fit_warned <- tryCatch(
  fetwfeWithSimulatedData(sim, lambda_selection = "bic", fusion_matrix = out_of_bounds),
...
`fetwfe()` also **stops** (rather than warns) when `fusion_matrix` is not a
finite, invertible numeric matrix of the exact `num_treats x num_treats`
dimension --- a malformed restriction matrix is a usage error, not a modeling
choice.
