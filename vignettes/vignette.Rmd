...
The fitted object also plays nicely with the [broom](https://broom.tidymodels.org/) package: `broom::tidy()` reshapes the overall ATT and per-cohort CATTs into a long data frame with standard `term` / `estimate` / `std.error` / `statistic` / `p.value` / `conf.low` / `conf.high` columns; `broom::glance()` returns a one-row model-level summary; and `broom::augment(res, data = pdata)` appends `.fitted` and `.resid` columns. This makes it easy to pipe estimates into `ggplot2` or `modelsummary` without reading the package's class documentation.

```{r tidy-demo, eval = requireNamespace("broom", quietly = TRUE) && requireNamespace("ggplot2", quietly = TRUE) && requireNamespace("dplyr", quietly = TRUE)}
library(broom)
...
library(ggplot2)
ggplot(tidy_res |> dplyr::filter(term != "ATT"),
...
`betwfe()` uses bridge regression directly on the coefficients (rather than on the fused restrictions used by `fetwfe()`). Under a different sparsity assumption (Kock 2013's bridge oracle property applied to the original coefficient vector), an analogous selection-consistency result holds, and `selected` carries similar (though weaker) information than for `fetwfe()`. For `etwfe()` and `twfeCovs()`, neither has a selection step, so the `selected` column is omitted; the `P_value` column is a standard post-OLS t-test.

Two finite-sample caveats are worth keeping in mind. First, even when `selected = TRUE`, the standard CI may cover zero --- the selection step says "the truth is nonzero" but the magnitude CI says "could be zero". These can disagree at finite `N`; report both and use judgment. Second, the asymptotic 100% CI of `{0}` interpretation is an `N -> infinity` statement. At finite `N` --- when an effect is small relative to noise and the BIC-selected `lambda` is large --- the estimator can zero out a *true* nonzero effect. In small samples, read `selected` and `P_value` together rather than relying on either alone.
