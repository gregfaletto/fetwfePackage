...
and asymptotic standard errors and Wald CIs follow Theorem `te.asym.norm.thm.gen.cond`(a) (Eq. (C.39)) in the appendix.
...
Each row gives the predicted $\hat\tau_{\mathrm{CATT}}(r, t, \boldsymbol{x})$, its standard error, and a Wald confidence interval at `1 - res_demo$alpha`. To collapse to a single number per `newdata` row, aggregate across cohorts and times --- for example, with `dplyr` taking a simple average across post-treatment effects:
...
preds |>
    ...
    dplyr::summarise(
        ...
        std.error = sqrt(mean(std.error^2)) / sqrt(dplyr::n()),
        .groups   = "drop"
    )
```
