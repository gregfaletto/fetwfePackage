...
The four available family options are `"event_study"` (one effect per post-treatment event time), `"cohort"` (one effect per treated cohort), `"all_post_treatment"` (one effect per `(r, t)` cell), and `"custom"` (a user-supplied `K x num_treats` contrast matrix following the `multcomp::glht()` convention; see `?simultaneousCIs`).
...
Only the interval bounds change. The standard errors (`se`), per-cohort p-values (`p_value`), selection flags (`selected`), and the overall-ATT confidence interval (a single scalar, with no family and hence no widening) are **identical** under both settings --- the only difference is the critical-value multiplier (`qnorm(1 - alpha/2)` for pointwise vs the joint $c_{1-\alpha}$ for simultaneous).
