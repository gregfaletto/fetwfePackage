...
- `betwfe()` — bridge-penalized ETWFE (without the fusion transformation that distinguishes `fetwfe()` from `betwfe()`).
...
A practical wrinkle: unlike `fetwfe()` and `etwfe()`, `betwfe()` does **not** currently provide a `summary()` S3 method. Its return value is a plain `list`, so we extract the fields directly:

```{r}
res_betwfe <- betwfeWithSimulatedData(sim_data)

cat("Estimated overall ATT:", res_betwfe$att_hat, "\n")
cat("Standard error:       ", res_betwfe$att_se, "\n")
cat("\nCohort-specific estimates:\n")
print(res_betwfe$catt_df)
```
